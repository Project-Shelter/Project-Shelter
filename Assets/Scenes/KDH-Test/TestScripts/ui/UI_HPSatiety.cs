...
public class UI_HPSatiety : UI_Section 
{
    ...
    {
        hpText.text = hpIndexText + ActorController.Instance.CurrentActor.Stat.HP.ToString() + "%";
    }

    private void GetPlayerSatiety()
    {
        satietyText.text = satietyIndexText + ActorController.Instance.CurrentActor.Stat.satiety.Value.ToString() + "%";
    }
}
